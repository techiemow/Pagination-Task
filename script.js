...
previous.setAttribute("id","MyPrevious")
previous.href = `#`;
...
previous.addEventListener('click', (e) => {
    ...
    let currentPageData = pages.find(p => p.id === currentPage.toString());
    if (currentPageData) {
        display.textContent = `ID:${currentPageData.id},Name:${currentPageData.name},Email:${currentPageData.email}`;
    }
});
...
pagination.appendChild(previous)
  ...
  first.setAttribute("id","MyButton")

  first.addEventListener('click', (e) => {
      e.preventDefault();
      page=pages[0]
      display.textContent = `ID:${page.id},Name:${page.name},Email:${page.email}`
...
// creating 1 to 100 Button
pages.forEach(page => {
    let pageLink = document.createElement('a');
    pageLink.href = `#${page.id}`;
    pageLink.textContent = `${page.id}`;
    pageLink.setAttribute("id","MyButton")
